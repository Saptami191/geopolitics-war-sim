...
Object.keys(SEED_TEMPLATES).forEach((id) => {
  ...
  INITIAL_COUNTRIES[id] = {
    ...
    haarpActive: false,
    domesticNarrative: t.ideology === 'DEMOCRACY' ? 55 : t.ideology === 'AUTOCRACY' ? 75 : 65,
    mediaResistance: Math.max(0.15, Math.min(0.95, (t.ideology === 'DEMOCRACY' ? 0.35 : t.ideology === 'AUTOCRACY' ? 0.75 : 0.55) + (t.power > 70 ? 0.15 : 0.05))),
    recentNarrativeDelta: 0,
    hasCivilUnrestTriggered: false,
    hasElectionInterferenceTriggered: false,
    provinces,
    populationSim,
    cabinet,
    economic: {
